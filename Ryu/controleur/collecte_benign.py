...
class CollectTrainingStatsApp(switch.SimpleSwitch13):
    ...
    def state_change_handler(self, ev):
        ...
        if ev.state == MAIN_DISPATCHER:
            if datapath.id not in self.datapaths:
                self.logger.debug('Enregistrement du datapath : %016x', datapath.id)
                self.datapaths[datapath.id] = datapath
        elif ev.state == DEAD_DISPATCHER:
            if datapath.id in self.datapaths:
                self.logger.debug('Désenregistrement du datapath : %016x', datapath.id)
    ...
    def request_stats(self, datapath):
        self.logger.debug('Envoi de la demande de statistiques : %016x', datapath.id)
        parser = datapath.ofproto_parser
        #req = parser.OFPFlowStatsRequest(datapath, 0, parser.OFPTT_ALL, ofproto_v1_3.OFPP_NONE)
        req = parser.OFPFlowStatsRequest(datapath, table_id=ofproto_v1_3.OFPTT_ALL)
