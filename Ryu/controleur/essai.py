...
class CollectTrainingStatsApp(switch.SimpleSwitch13):
    def __init__(self, *args, **kwargs):
        ...
        # Ouvrir les fichiers CSV en mode append
        self.open_csv_files()

    def open_csv_files(self):
        # Ouvrir les fichiers CSV une fois à l'initialisation
        self.file0 = open('FlowStatsfile.csv', 'a+')
        self.file1 = open('PortStatsfile.csv', 'a+')

        # Écrire les en-têtes des fichiers CSV si les fichiers sont vides
        if os.stat('FlowStatsfile.csv').st_size == 0:
            self.file0.write('timestamp,datapath,flow_id,in_port,eth_type,eth_src,eth_dst,ip_src,tp_src,ip_dst,tp_dst,'
                             'icmp_code,icmp_type,protocol,pktcount,bytecount,flowdur_sec,flowdur_nsec,pktcount_sec,'
                             'pktcount_nsec,bytecount_sec,bytecount_nsec,label\n')

        if os.stat('PortStatsfile.csv').st_size == 0:
            self.file1.write('timestamp,datapath,rx-pkts,rx-bytes,rx-error,rx_dropped,tx-pkts,tx-bytes,tx-error,'
                             'tx_dropped,duration_nsec,label\n')
        ...
        while True:
            with concurrent.futures.ThreadPoolExecutor() as executor:
                for dp in self.datapaths.values():
                    executor.submit(self.request_stats, dp)  # Envoyez des demandes de statistiques en parallèle
            hub.sleep(10)
