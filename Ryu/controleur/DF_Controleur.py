...
class SimpleMonitor13(switch.SimpleSwitch13):
    ...
    def _state_change_handler(self, ev):
        # Gestionnaire d'événement pour les changements d'état du commutateur
        datapath = ev.datapath
        if ev.state == MAIN_DISPATCHER:
            if datapath.id not in self.datapaths:
                self.datapaths[datapath.id] = datapath
                self.logger.debug(f"Datapath {datapath.id:016x} connecté", )
        elif ev.state == DEAD_DISPATCHER:
            if datapath.id in self.datapaths:
                del self.datapaths[datapath.id]
                self.logger.debug(f"Datapath {datapath.id:016x} déconnecté", )


    async def _monitor(self):
        while True:
            await asyncio.gather(*(self._request_stats(dp) for dp in self.datapaths.values()))
            await asyncio.sleep(10)

            self.flow_predict()


    async def _request_stats(self, datapath):
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        for stat in sorted([flow for flow in body if (flow.priority == 1) ], key=lambda flow:
            ...
            file0.write(f"{timestamp},{ev.msg.datapath.id},{flow_id},{ip_src},{tp_src},{ip_dst},{tp_dst},"
                        f"{stat.match['ip_proto']},{icmp_code},{icmp_type},{stat.duration_sec},{stat.duration_nsec},"
    ...
    def flow_training(self):
        ...
        # Supprimer les points dans les colonnes spécifiques (2, 3, 5)
        columns_to_clean = [2, 3, 5]
        flow_dataset.iloc[:, columns_to_clean] = flow_dataset.iloc[:, columns_to_clean].replace('\.', '', regex=True)
